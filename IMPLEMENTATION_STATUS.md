...
- 回归：`./tools/run_regression.sh` 当前输出 `REGRESSION_OK tests=25`；Linux Vulkan PCK smoke 已通过，
  Windows EXE 已按收尾代码重新导出；仍需在真实 Windows 驱动/输入/全屏环境走查。
- 收尾视觉：近景树冠填充、外翻树根、地表植物图集、角色轮廓补光、湖面传输/反射和 Windows PCK
  资源白名单已固化；本提交后不再继续扩展功能。
...
- 当前 PCK SHA-256 为 `e15ee9de223a8715e6d4f3eaf6d97f01c356490fad1eae079608028b5e85b01f`（541,289,800 bytes）；
- ZIP SHA-256 为 `4b77d92bab1d91165b52c8d23d0b4969dee7d02ca81eb96f37eb49cc45b3e4da`（578,607,845 bytes）；
- 不含签名、安装器、自动更新和崩溃上报；
...
源码、玩法和美术功能在本提交后冻结。剩余工作仅限目标 Windows 设备的启动、驱动、全屏、输入、
物理交互和性能记录；不得把未在本机验证的 Windows 实机结果写成已通过，也不再向仓库追加新的功能线。
