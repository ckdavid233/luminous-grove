...
func _initialize() -> void:
	...
	assert(is_equal_approx(far_tree_batch.visibility_range_begin, 30.0))
	assert(is_equal_approx(far_tree_batch.visibility_range_end, 72.0))
	assert(is_equal_approx(mid_tree_batch.visibility_range_begin, 12.0))
	assert(is_equal_approx(mid_tree_batch.visibility_range_end, 36.0))
	...
	assert(grass.cast_shadow == GeometryInstance3D.SHADOW_CASTING_SETTING_OFF)
	assert(is_equal_approx(far_tree_batch.visibility_range_begin, 27.0))
	assert(is_equal_approx(far_tree_batch.visibility_range_end, 60.0))
	assert(is_equal_approx(mid_tree_batch.visibility_range_begin, 11.0))
	assert(is_equal_approx(mid_tree_batch.visibility_range_end, 32.0))
	...
	assert(tree_batch.cast_shadow == GeometryInstance3D.SHADOW_CASTING_SETTING_OFF)
	assert(is_equal_approx(far_tree_batch.visibility_range_begin, 23.0))
	assert(is_equal_approx(far_tree_batch.visibility_range_end, 46.0))
	assert(is_equal_approx(mid_tree_batch.visibility_range_begin, 9.0))
	assert(is_equal_approx(mid_tree_batch.visibility_range_end, 27.0))
