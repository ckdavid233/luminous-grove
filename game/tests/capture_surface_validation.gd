...
func _capture_tree() -> void:
	...
	var far_canopies := _main.get_node_or_null("Forest/FarTreeCanopies")
	if trees_two != null:
	...
	if mid_canopies != null:
		mid_canopies.visible = false
	if far_trunks != null:
		far_trunks.visible = false
	if far_canopies != null:
		far_canopies.visible = false
