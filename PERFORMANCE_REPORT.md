...
真实 Vulkan X11 采样中，高画质档为 8.4 FPS，性能档为 17.6 FPS；当前窗口/驱动组合下，
高画质只适合画面验收，集显游玩仍需进一步降低反射、阴影和渲染比例。

本次林地湖区材质、视觉、光照与交互改造已经完成代码和资源接入，并完成一次真实 Vulkan
复测。高质量目标 P95 ≤ 41.7 ms 在 Renoir/X11 上未达成：高画质 P95 120.904 ms，平衡
P95 100.224 ms，性能 P95 59.794 ms。该结果是真实渲染证据，不应被误读为目标已通过。
...
|---|---:|---:|---:|
| 样本帧 | 600 | 600 | 600 |
| 平均 FPS | 8.4 | 10.3 | 17.6 |
| 平均帧时间 | 118.444 ms | 97.556 ms | 56.963 ms |
| P95 | 120.904 ms | 100.224 ms | 59.794 ms |
| P99 | 123.745 ms | 110.850 ms | 106.463 ms |
