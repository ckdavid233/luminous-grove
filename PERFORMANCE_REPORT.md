...
收尾代码又加入近景树冠填充、外翻树根、地表植物图集、角色轮廓光和发布 PCK 图集白名单；本轮没有
伪造新的 600 帧性能矩阵。重复 Vulkan 捕获在 Renoir 环境出现约 452 MB 临时资源分配失败并被系统
终止，因此本报告继续保留既有真实测量，未把捕获失败写成性能通过或 Windows 结果。功能与代码在本
提交后冻结，后续只接受目标 Windows 设备的实机性能记录。
