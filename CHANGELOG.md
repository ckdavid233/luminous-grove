...
- 试玩反馈修复：禁用运行时不推进骨骼的 AnimationTree 输出，改用物理时钟直接播放 GLB
  AnimationPlayer；新增骨骼位移回归断言，角色 Walk/Run/Jump/Fall/Land/Interact 不再停在静态姿势。
- 掉出场景保护：检查点增加有限值/高度校验，低于虚空阈值自动恢复位置、速度、交互状态和输入，Main
  通过 `void_recovered` 提示玩家；Jolt 配重踏板增加 0.08 秒重叠扫描，石头离开后返回仍能触发。
- 档案谜题升级为 campaign v8：新增三环 `ArchiveCipherConsole`（R 转动、E 确认），按 alignment 读取
  语义线索，错误确认可恢复并重置；完成符文校准后才开放倒影、配重和铭名透镜。
- 湖面高画质：增强双层法线、深度吸收、折射/反射、双层焦散、太阳闪光和岸线泡沫；雨滴 220、
  水面闪烁 180、飞溅 5 层环/高密度水滴，保持 32 槽水纹事件接口兼容。
- 回归套件增加 `archive_cipher_test.gd`，全量结果升级为 `REGRESSION_OK tests=25`；发布自检标记为
  `campaign=8`。

- 玩家反馈修复：推石头完成配重后，返回此岸会强制同步铭名透镜的碰撞与可用状态，并刷新玩家
  交互目标；`request_interaction()` 会丢弃跨时相留下的失效目标，避免透镜软锁。
- 档案锚点基础顺序（campaign v7）随 alignment 分支变化，错误顺序不会消耗锚点；残句线索
  与回归测试覆盖错误尝试、重试和配重→透镜完整路径。
- 角色视觉补强：运行时加入呼吸、步态重心、起停惯性和转身侧倾，并恢复服装/鞋的 authored normal
  map；森林中远景代理改为分层枝条和 5/4 簇状树冠，减少重复圆球树冠。
...
- 森林树木改为近景扫描 GLB、中景分层枝条与五簇树冠、远景两层枝条与四簇树冠的实际几何 LOD；新增
